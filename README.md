...
**Status: complete.** Seven build phases plus a Phase 8 for tooling: ingestion, async pipeline, PR fetch, a four-agent graph with deterministic aggregation, a unified Postgres data layer with pgvector search, idempotent posting behind a confidence gate, observability with tracing and verified cost accounting, bounded retries and a spend ceiling — and a GitHub Actions pipeline running lint, import checks, and a unit-test suite on every pull request. Every phase is broken into small, individually tested steps — see the [Build log](#build-log) for the full brick-by-brick decomposition, which doubles as a study map of the project.
...
### Phase 8 — CI and tests ✅
*Files: `.github/workflows/ci.yml`, `ruff.toml`, `pytest.ini`, `requirements-dev.txt`, `tests/`*

Tooling around the finished system rather than a further feature — hence a plain name instead of the phase-numbered commits used above.

- **8a — Lint + import checks ✅.** A GitHub Actions workflow triggered on pushes to `main` and on every pull request: install pinned deps, run `ruff` (rules `E`, `F`, `I`, `W`; `E501` ignored because the agent prompts are long single-line strings that break badly), then import every `app` module to catch anything that fails at load. Dev tooling lives in a separate `requirements-dev.txt` — a linter and test runner are not runtime dependencies. The first lint run immediately surfaced a duplicate `GitHubError` import, two unused imports, and — via the unused import — the dead `mark_failed()` function superseded by `record_failure()` in 7c. None of these broke anything at runtime, which is precisely why manual testing had never found them.
- **8b — Unit tests for the deterministic core ✅.** `pytest.ini` scopes collection to `tests/`, since the pre-existing `scripts/test_*.py` files are manual harnesses that need API keys and must not run in CI. Sixteen tests cover the two policies that matter most: failure classification (transient vs permanent, including the conservative default for unrecognized exceptions) and confidence tiering (noise floor, demote-don't-drop, dedupe, severity ordering, and both threshold boundaries). Writing them forced a design change: the tiering logic was extracted out of the `aggregator` node into a pure `tier_findings()` so it could be tested without an LLM call, with behavior verified unchanged against a real PR afterward. A `pytest` step was added to the workflow — closing the exact gap the system's own review had flagged (see [below](#a-note-on-the-reviewer-reviewing-itself)).
- **8c — Integration tests against Postgres 🔲.** The database layer still has no coverage; see [Known gaps](#known-gaps-deliberately-deferred).
